...
# 만약 식탁에 [식빵, 식빵, 식빵, 토마토, 식빵, 식빵, 토마토, 식빵, 토마토, 식빵] 
# 순으로 놓여 있다면 현수는 3번째, 4번째, 5번째를 합쳐서 샌드위치를 만들고, 
# 6번째, 7번째, 8번째를 합쳐서 샌드위치를 만들고, 
# 2번째, 9번째, 10번째를 합쳐서 샌드위치를 만들어 총 3개의 샌드위치를 만들 수 있습니다.
...
def solutions(nums):
    ...
    for i in nums:
        if len(stack) > 2 and i == 1 and stack[-2] == 1 and stack[-1] == 2:
            answer += 1
            stack.pop()
            stack.pop()
        else:

...
# 각 격자에는 그 지역의 높이가 쓰여있습니다. 각 지역은 상하좌우 인접한 지역의 숫자가 모두
...
def solutions(nums):
    ...
    for i in range(n):
        for j in range(n):
            flag = True #웅덩이라고 처음부터 가정 / 처음부터 한인 티 X
            for k in range(4):
                nx = i + dx[k]
                ny = j + dy[k]
                # -1 인덱스는 없으니까, out of range 방지하기 위함
                # nx, ny는 0부터 n-1까지 존재할때만 되도록 설정
                # 이러한 설정은 무조건 앞쪽에다가 설정할 것
